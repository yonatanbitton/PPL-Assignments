  ...
  (lambda (exp env)
    ...
    (cond ((num-exp? exp) (num-exp->val exp))
          ...
          ((app-exp? exp)
           ...
           (let ((rator (if (var-ref? (app-exp->rator exp)) (apply-env env (second (app-exp->rator exp))) (app-exp->rator exp))))
             (cond ((closure? rator) 
                                     (let ((lazies (map (lambda (var) (if (var-decl? var) #f #t)) (closure->params rator))))
                  
                                       (let ((updated (letrec ((loop (lambda (rands lazies acc)
                                               
                                                                       (if (not (empty? rands))
                                                                           (if (eq? #f (car lazies))
                                                                               (loop (cdr rands) (cdr lazies) (append acc (list (L3-applicative-eval (car rands) env))))
                 ...
                 (else (L3-apply-procedure (L3-applicative-eval (app-exp->rator exp) env)
                               (map (lambda (rand) (L3-applicative-eval rand env))
                                    (app-exp->rands exp))
                               env)))  )
           )
...
(define L3-apply-procedure
  (lambda (proc args env)
    ...
    (cond ((prim-op? proc)
           (L3-apply-primitive proc args))
          ((closure? proc)
           (let ((vars (map (lambda (var) (if (var-decl? var) (var-decl->var var) (var-decl-lazy->var var))) (closure->params proc)))
                 (body (L3-rename-exps (closure->body proc))))
          
             (let ((lit-args (map (lambda (arg) (if (list? arg) (if (cexp3? arg) arg (L3-value->lit-exp arg)) (L3-value->lit-exp arg))) args)))              
               (L3-eval-sequence (L3-substitute body vars lit-args) env))))
                    ...
                    ((proc-exp? exp)
                     (let ((old-params (map var-decl->var (proc-exp->params exp)))
                       ...
                       (let ((new-param-names (map gen-var old-params))
                         ...
                         (make-proc-exp (map (lambda (par decl-par) (if (var-decl-lazy? decl-par) (make-var-decl-lazy (list par))
                                                                  (make-var-decl par)))
                                                                  new-param-names (proc-exp->params exp))
