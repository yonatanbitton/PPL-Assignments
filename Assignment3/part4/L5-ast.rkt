...
(define composite-te?
  (lambda (x) (or (proc-te? x) (pair-te? x) )))
...
;; Purpose: Pair-te value constructor
;; Type: [texp * texp -> pair-te]
(define make-pair-te
  (lambda (first-tes second-te)
    (list 'pair-te first-tes second-te)))
(define pair-te?
  (lambda (x) (and (list? x) (= (length x) 3) (eq? (car x) 'pair-te))))
(define pair-te->first-te
  (lambda (pair-te) (second pair-te)))
(define pair-te->second-te
  (lambda (pair-te) (third pair-te)))
...
;; symbol-type
;; Purpose: symbol-type value constructor
;; Signature: make-symbol-type(val)
;; Type: [Sexp -> symbol-type]
(define make-symbol-te
  (lambda (val)
    'symbol-te))
(define symbol-te?
  (lambda (x)
    (eq? x 'symbol-te)))
(define symbol-te->val
  (lambda (symbol) (second symbol)))
...
(define parse-texp
  (lambda (texp)
    ...
    (if (and (list? texp) (= (length texp) 3) (or (eq? (first texp) 'pair) (eq? (first texp) 'Pair)))
        (make-pair-te (parse-texp (second texp)) (parse-texp (third texp)))
    (cond ((eq? texp 'number) (make-num-te))
          ...
          ((eq? texp 'Void) (make-void-te))
          ((eq? texp 'symbol) (make-symbol-te 'x))
          ((eq? texp 'Symbol) (make-symbol-te 'x))
...
;(parse-texp '(T * T -> boolean))
 
 
;(parse-texp '((pair 'quote a))) ;-> (lit-exp-type a))
  ...
  (lambda (te)
    ...
    (letrec ((unparse-tuple (lambda (param-tes)
      ...
      (cond ((eq? te 'num-te) 'number)
            ...
            ((eq? te 'void-te) 'void)
            ((or (eq? te 'symbol-te) (equal? te '(symbol-te))) 'symbol)
            ((tvar? te)
            ...
            ((pair-te? te)
             ...
             (list 'Pair (unparse-texp (second te)) (unparse-texp (third te)))
             )
            ((proc-te? te)
                         ...
                         (cond ((eq? (car x) 'L5)
                               ...
                               ((eq? (car x) 'define)
                                (make-def-exp (parse-var-decl (second x))
                                              (parse (third x))))
                               ((or (eq? (car x) 'pair) (eq? (car x) 'Pair))
                                (make-pair-te (parse-texp (second x))
                                             (parse-texp (third x))
                                             ))
                               ((eq? (car x) 'if)
...
(define unparse
  (lambda (exp)
    (cond ((num-exp? exp) (num-exp->val exp))
          ...
          ((lit-exp? exp) (list 'quote (lit-exp->val exp)))
