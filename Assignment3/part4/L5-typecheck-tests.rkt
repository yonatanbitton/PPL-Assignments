...
(define myLetPairTests (lambda ()
                        (letrec ((do (lambda (exp) (unparse-texp (typeof-exp (parseL5 exp) (make-empty-tenv))))))                         
                          ...
                          (test (do '(letrec (([p1 : ((Pair boolean number) -> number)] (lambda ([x : (Pair boolean number)]) : number 2)))
                           ...
                           #;(test (do '(letrec (([p1 : ((Pair boolean number) -> number)] (lambda ([x : (Pair boolean number)]) : number (cdr p1))))
                              p1)) => '((Pair boolean number) -> number))
))))
...
(define myQuoteTests
  (lambda ()
    (letrec ((do (lambda (exp) (unparse-texp (typeof-exp (parseL5 exp) (make-empty-tenv))))))                         
                          (run-tests
                          #;(test (do '(let (([x : number] (car (cons 1 2)))) #f)) => 'boolean)
(define q1 (parseL5 '(quote 2)))
(test (typeof-exp q1 (make-empty-tenv))  => 'num-te)

(define q2 (parseL5 '(quote x)))
(test (typeof-exp q2 (make-empty-tenv)) => 'symbol-te)

(define q3 (parseL5 '(quote (1 . 2))))
(test (typeof-exp q3 (make-empty-tenv)) => '(pair-te num-te num-te))

;(define q4 (parseL5 '(quote (1 . (2 . #f)))))
;(typeof-exp q4 (make-empty-tenv)); => '(pair-te num-te (pair-te num-te bool-te))
    ))))
