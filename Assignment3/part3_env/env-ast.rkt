...
;; Purpose: Value constructor for var-decl-lazy
;; Type: [Symbol -> var-decl-lazy]
;; Signature: make-var-decl-lazy (Symbol)
(define make-var-decl-lazy
  (lambda (var) (list 'var-decl-lazy (first var))))

;; Purpose: Type predicate for var-decl-lazy
;; Type: [CExp -> Boolean]
;; Signature: var-decl-lazy? (CExp)
(define var-decl-lazy?
  (lambda (x) (and (list x) (= (length x) 2) (eq? (car x) 'var-decl-lazy))))

;; Purpose: var accessor for var-decl-lazy's var
;; Type: [var-decl-lazy -> Symbol]
;; Signature: var-decl-lazy (var-decl-lazy)
(define var-decl-lazy->var
  (lambda (var-decl) (second var-decl)))
           ...
           (cond ((eq? (car x) 'L4)
                 ...
                 ((eq? (car x) 'if)
                  (make-if-exp (parseL4 (second x))
                               ...
                               (parseL4 (fourth x))))
                 ((eq? (car x) 'lambda)
                  (make-proc-exp (map (lambda (dec) (if (and (list? dec) (eq? (second dec) 'lazy))
                                                        (make-var-decl-lazy dec)
                                                        (make-var-decl dec))) (second x))
                                 (map parseL4 (cddr x))))
                 ((eq? (car x) 'let)
