...
;; Thunk
;; =====
;; Purpose: Value constructor for thunk
;; Signature: make-thunk(vars vals)
;; Type: [SExp * Env -> Thunk]
(define make-thunk
  (lambda (exp env) (list 'thunk-exp exp env)))
#;(define thunk-exp?
  (lambda (x) (and (list? x)(= (length x) 3) (eq? (car x) 'thunk-exp))))
;; Purpose: Type predicate for thunk
;; Signature: thunk?(CExp)
;; Type: [CExp -> Boolean]
(define thunk?
  (lambda (x) (and (list? x)(= (length x) 3) (eq? (car x) 'thunk-exp))))

;; Purpose: Value accessor for thunk's inner exp
;; Signature: thunk-exp->exp(thunk-exp)
;; Type: [Thunk -> SExp]
(define thunk-exp->exp
  (lambda (thunk-exp) (second thunk-exp)))

;; Purpose: Value accessor for thunk's environment
;; Signature: thunk-exp->env(thunk-exp)
;; Type: [Thunk -> Env]
(define thunk-exp->env
  (lambda (thunk-exp) (third thunk-exp)))
...
(define myThunk
(make-thunk '(num-exp 5) (make-global-env)))
;myThunk
